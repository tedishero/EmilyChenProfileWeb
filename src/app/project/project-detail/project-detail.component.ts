...
const PhotoSwipeUI_Default = function(pswp, framework) {
  const ui = this;
  ...
  let _overlayUIUpdated = false,
    ...
    _idleIncrement = 0,
    _idleTimer;
  const _togglePswpClass = function(el, cName, add) {
    framework[(add ? 'add' : 'remove') + 'Class'](el, 'pswp__' + cName);
  };
  const _toggleShareModalClass = function() {
    _togglePswpClass(_shareModal, 'share-modal--hidden', _shareModalHidden);
  };
  const _toggleShareModal = function() {
  ...
  const _toggleLoadingIndicator = function(hide) {
    if (_loadingIndicatorHidden !== hide) {
      _togglePswpClass(_loadingIndicator, 'preloader--active', !hide);
      _loadingIndicatorHidden = hide;
    }
  };
  const _onIdleMouseMove = function() {
  ...
  const _onControlsTap = function(e) {
...
export class ProjectDetailComponent implements OnInit {
  ...
  private setMetaTags() {
    ...
    this.meta.updateTag({
      property: 'og:image',
      content: `https:${this.project.images[0].url}`
    });
    this.meta.updateTag({
    ...
    this.meta.updateTag({
      ...
      content: this.project.images[0].height.toString()
    });
    this.meta.updateTag({
      property: 'og:image:width',
      content: this.project.images[0].width.toString()
    });
  ...
  ngOnInit() {
    const pageData: ProjectDetailPageData = this.route.snapshot.data['pageData'];
    this.project = {
  ...
  public compressedDetailImage(projectImg: ProjectImage): string {
    const fileNamePrefix = projectImg.fileName.split('.')[0];
  ...
  openImagePopUp(imgIndex: number) {
    ...
    const gallery = new PhotoSwipe(pswpElement, PhotoSwipeUI_Default, this.popupImages, options);
    gallery.init();
  }
  ...
  public onlazyImgLoad(index: number) {
    ...
    const img: HTMLElement = document.getElementById(`project-detail-img-${index}`);
